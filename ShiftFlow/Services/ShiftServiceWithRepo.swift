...
protocol ShiftServiceProtocol {
    ...
    /// Fetch next page of shifts for pagination
    func fetchNextShiftsPage(for companyId: String) async throws
    
    /// Fetch next page of user shifts for pagination
    func fetchNextUserShiftsPage(for userId: String, in companyId: String) async throws
}
...
class ShiftServiceWithRepo: ObservableObject, ShiftServiceProtocol {
    ...
    private var lastUserShiftDocument: DocumentSnapshot? = nil
    ...
    func fetchShifts(for companyId: String) async throws -> [Shift] {
        ...
        do {
            ...
            await MainActor.run {
                ...
                self.hasMorePages = fetchedShifts.count >= self.pageSize
            }
    ...
    func fetchNextShiftsPage(for companyId: String) async throws {
        ...
        do {
            // For now, we'll implement a simple pagination approach
            // In a real implementation, you'd want to modify the repository to support pagination
            let allShifts = try await shiftRepository.getShiftsForCompany(companyId: companyId)
            let currentCount = shifts.count
            let nextPageShifts = Array(allShifts.dropFirst(currentCount).prefix(pageSize))
            
            await MainActor.run {
                if !nextPageShifts.isEmpty {
                    self.shifts.append(contentsOf: nextPageShifts)
                }
                self.hasMorePages = nextPageShifts.count >= self.pageSize && (currentCount + nextPageShifts.count) < allShifts.count
                self.isLoadingMore = false
            }
        } catch {
            await MainActor.run {
                self.errorMessage = "Failed to fetch more shifts: \(error.localizedDescription)"
                self.isLoadingMore = false
            }
            throw error
        }
    ...
    func fetchNextUserShiftsPage(for userId: String, in companyId: String) async throws {
        ...
        do {
            // For now, we'll implement a simple pagination approach
            // In a real implementation, you'd want to modify the repository to support pagination
            let allShifts = try await shiftRepository.getShiftsForUser(userId: userId, companyId: companyId)
            let currentCount = shifts.count
            let nextPageShifts = Array(allShifts.dropFirst(currentCount).prefix(pageSize))
            ...
            await MainActor.run {
                self.errorMessage = "Failed to fetch more user shifts: \(error.localizedDescription)"
                self.isLoadingMore = false
            }
            throw error
        }
